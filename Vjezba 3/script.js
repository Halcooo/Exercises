...
function renderBlogs(blogs) {
  ...
  for (var blog of blogs) {
    ...
    div.appendChild(dislike(blog));
    div.appendChild(like(blog));
...
like.classList.add("fa");
  ...
  counter.innerText = blog.likes.length;

  likeWrapper.appendChild(like);
  likeWrapper.appendChild(counter);
  
  like.addEventListener("click", function () {
    if (!blog.likes.includes(loggedUser.name)) {
      blog.likes.push(loggedUser.name);
      localStorage.setItem("blogs", JSON.stringify(allBlogs));
      displayBlog();
    }
  });
...
function dislike(blog) {
  var dislikeWrapper = document.createElement("div");
  dislikeWrapper.classList.add("like-wrapper");
  ...
  counter.innerHTML = blog.dislikes.length;

  dislikeWrapper.appendChild(dislikeBtn);
  dislikeWrapper.appendChild(counter);
  dislikeBtn.addEventListener("click", function () {
    if (!blog.dislikes.includes(loggedUser.name)) {
      blog.dislikes.push(loggedUser.name);
      localStorage.setItem("blogs", JSON.stringify(allBlogs));
